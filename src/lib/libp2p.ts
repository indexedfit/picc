...
export type Libp2pType = Libp2p<{
  pubsub: PubSub;
  identify: Identify;
  // directMessage: DirectMessage; add this back for direct 1-to-1 comms.
...
export async function startLibp2p(): Promise<Libp2pType> {
  ...
  libp2p.services.pubsub.subscribe(FILE_TOPIC);
  ...
  //
  //
  // note, in this case, maybe we should be conservative and not actually dial anyone before sync?
...
async function getRelayListenAddrs(
